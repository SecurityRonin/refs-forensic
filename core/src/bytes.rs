...
//!
//! ReFS is a **little-endian** on-disk format (Windows/NT platform), so these
//! mirror `xfs-core`'s big-endian helpers with LE decode. Every reader yields
//! `0` when the requested range lies outside the buffer, so a malformed or
//! truncated image can never panic a parser. Callers that need to distinguish
